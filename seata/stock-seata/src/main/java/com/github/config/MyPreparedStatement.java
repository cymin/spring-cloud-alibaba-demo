...
public class MyPreparedStatement implements PreparedStatement{
    ...
    public boolean execute() throws SQLException {
        ...
        int exeCount = MyConnection.getExeCount(connection);
        if (exeCount > 2){
            System.out.println("sql 太多，插入失败 " + MyConnection.getExeCount(connection) );
            connection.rollback();
            return false;
        }else {
            MyConnection.increase(connection);
            System.out.println("执行了" + MyConnection.getExeCount(connection) + "条sql");
            return ps.execute();
        }
    }
    ...
    public MyPreparedStatement(PreparedStatement ps){
        this.ps = ps;
    }
}
